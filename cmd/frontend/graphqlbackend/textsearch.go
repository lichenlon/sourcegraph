		...
		for _, repoRev := range searcherRepos {
			...
			if args.PatternInfo.IsStructuralPat && searcherReposFilteredFiles != nil {
				...
				if v, ok := searcherReposFilteredFiles[string(repoRev.Repo.Name)]; ok {
					...
					threshold := 100 // if there are more matching files than this threshold, only search the first 100.
					if len(v) > threshold {
						args.PatternInfo.IncludePatterns = append(includePatternsCopy, v[:100]...)
					} else {
